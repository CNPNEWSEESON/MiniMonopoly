...
export const createChanceDeck = (): ChanceCard[] => [
    ...
    {
        title: "-$100", description: "Pay an unexpected bill.", apply: (p, c) => {
            c.payTax(p, 100);
            return `${p.name} paid $100.`;
        }
    },
    {
        title: "Forward", description: "Move forward 3 spaces.", apply: (p, c) => {
            c.move(p, 3);
            return `${p.name} moved forward 3 spaces.`;
        }
    },
    {
        title: "Backward", description: "Move backward 2 spaces.", apply: (p, c) => {
            c.move(p, -2);
            return `${p.name} moved backward 2 spaces.`;
        }
    },
    {
        title: "Tax", description: "Pay $75 tax.", apply: (p, c) => {
            c.payTax(p, 75);
